...
class Game {
	...
	async init() {
		...
		try {
			...
			await this._audio.loadAudio(this._songMeta.audioPath, (progress) =>
				setLoadingProgress(
					0.3 + progress * 0.6,
					`正在加载音频... ${Math.round(progress * 100)}%`
				)
			);
	...
	_handleTap(e) {
		...
		if (import.meta.env?.DEV || window.DEBUG_MODE) {
			console.log(
				`[判定] ${result.rating} | 偏差: ${result.diff.toFixed(1)}ms | Combo: ${combo}`
			);
		}
	}
